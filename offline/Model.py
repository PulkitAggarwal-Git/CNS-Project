...
class FeatureNet(nn.Module):
    def __init__(self, ftr_len, num_label):
        super().__init__()
        # Branch 1: processes input sequence statistics
        self.input_branch = nn.Sequential(
            nn.Linear(ftr_len // 2, 64),
            nn.ReLU(),
            nn.Linear(64, 32)
        )
        # Branch 2: processes output sequence statistics
        self.output_branch = nn.Sequential(
            nn.Linear(ftr_len // 2, 64),
            nn.ReLU(),
            nn.Linear(64, 32)
        )
        # Branch 3: processes relationship between input/output
        self.relation_branch = nn.Sequential(
            nn.Linear(ftr_len, 64),
            nn.ReLU(),
            nn.Linear(64, 32)
        )
        # Final classifier
        self.classifier = nn.Linear(32 + 32 + 32, num_label)
    
    def forward(self, x, x_lens, y, y_lens, feature_data):
        mid = feature_data.size(1) // 2
        in_features = feature_data[:, :mid]
        out_features = feature_data[:, mid:]
        
        in_out = self.input_branch(in_features)
        out_out = self.output_branch(out_features)
        rel_out = self.relation_branch(feature_data)
        
        combined = torch.cat([in_out, out_out, rel_out], dim=1)
